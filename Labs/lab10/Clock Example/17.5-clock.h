...
class Clock{
 ...
 public:
  // constructors
 Clock() : minutes_(0), hours_(0), period_("AM") {cout << "default ctor" << endl;};
  Clock(int, int, string);
  explicit Clock(string s);
  Clock(const Clock&); // copy constructor
  ~Clock() {cout << "dtor" << endl;};         // destructor. Does nothing yet
  Clock(int i): minutes_(i), hours_(0), period_("AM") {cout <<"1int ctor" << endl;} ; 
